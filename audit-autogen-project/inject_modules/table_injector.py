...
def inject_tables_and_summary(output_path, template_path, summary_values, dest_path, alias_dict=None):
    wb_src = load_workbook(output_path, data_only=True)
    wb_tgt = load_workbook(template_path)
    ws_tgt = wb_tgt.active

    # ✅ 构造 df_map 配置（来自 mapping_file.xlsx）
    mapping_file = str(template_path.parent / "mapping_file.xlsx")
